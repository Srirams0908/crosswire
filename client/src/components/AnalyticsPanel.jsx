...
export default function AnalyticsPanel({ data }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card">
        <h3 className="font-display text-base font-bold text-white mb-1">Handoff Notes</h3>
        <p className="text-xs text-navy-400 mb-4">Average word count per team · on-time vs auto-saved</p>

        {handoffByTeam.length === 0 ? (
          <p className="text-sm text-navy-500 italic">No handoff notes recorded.</p>
        ) : (
        ...
        <div className="flex items-center gap-4 mt-4 pt-4 border-t border-navy-700">
          ...
          <Legend dot="bg-amber-500/60" label="Auto-saved at round end" />
          <Legend dot="bg-navy-600" label="Not written" />
        </div>
      </div>

      {/* ── Reflection response rates ── */}
      <div className="card">
        <h3 className="font-display text-base font-bold text-white mb-1">Reflection Response Rate</h3>
      ...
      {/* ── Word frequency ── */}
      <div className="card">
        <h3 className="font-display text-base font-bold text-white mb-1">Top Words in Workspace Content</h3>
        <p className="text-xs text-navy-400 mb-4">Most frequent words written across all teams and rounds</p>

        {topWords.length === 0 ? (
          <p className="text-sm text-navy-500 italic">No workspace content recorded.</p>
        ) : (
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="w-28 flex-shrink-0 flex items-center gap-1.5">
        ...
        <div className="min-w-0">
          <div className="text-xs font-medium text-white truncate">{team.country}</div>
          {team.instanceNum > 1 && (
            <div className="text-xs text-navy-500">Inst. {team.instanceNum}</div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center gap-2">
        <div className="flex-1 h-5 bg-navy-800 rounded-full overflow-hidden">
          <div
            className="h-full bg-amber-500/70 rounded-full transition-all duration-500 flex items-center pl-2"
            style={{ width: `${Math.max(barPct, 2)}%` }}
          />
        </div>
        <span className="text-xs font-mono text-navy-300 w-12 text-right">
          {team.avgWordCount} {team.avgWordCount === 1 ? 'word' : 'words'}
        </span>
      </div>

      {/* Per-note submission dots */}
      <div className="flex gap-1 flex-shrink-0">
        {team.notes.map((note, i) => (
          <NoteStatusDot key={i} note={note} roundLabel={`R${note.round}`} />
        ))}
        {/* Pad to 2 slots if team only wrote 1 note (e.g. last round = no handoff) */}
        {Array.from({ length: Math.max(0, 2 - totalSlots) }).map((_, i) => (
          <div key={`pad-${i}`} className="w-6 h-6 rounded-full bg-navy-800 border border-navy-700 flex items-center justify-center">
...
function NoteStatusDot({ note, roundLabel }) {
  const bg = note.onTime
    ? 'bg-emerald-500'
    : note.hasContent
    ? 'bg-amber-500/60'
    : 'bg-navy-600';
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <span className={`text-xs ${bold ? 'font-medium text-navy-200' : 'text-navy-400'} truncate mr-2`}>
          {label}
        </span>
        <span className={`text-xs font-mono flex-shrink-0 ${bold ? 'text-white' : 'text-navy-400'}`}>
          {value}/{total} <span className="text-navy-600">({pct}%)</span>
        </span>
      </div>
...
function WordRow({ word, count, max }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs font-mono text-navy-200 w-24 truncate">{word}</span>
      ...
      <span className="text-xs text-navy-500 font-mono w-6 text-right">{count}</span>
    </div>
  );
}
