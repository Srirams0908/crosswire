  ...
  return (
    <div className="min-h-screen bg-gray-50 px-6 py-12">
      <div className="max-w-2xl mx-auto">
        {/* Header */}
        <div className="mb-10">
          <button onClick={() => navigate('/')} className="text-gray-400 hover:text-gray-700 text-sm flex items-center gap-2 mb-6 transition-colors">
            ← Back
          </button>
          <div className="flex items-center gap-3 mb-2">
            <span className="font-display text-2xl font-bold text-gray-900">CrossWire</span>
            <span className="text-gray-300">·</span>
            <span className="text-gray-500">Facilitator Setup</span>
          </div>
          {/* Step indicators */}
          <div className="flex items-center gap-2 mt-4">
            {[1, 2, 3].map(s => (
              <div key={s} className="flex items-center gap-2">
                ...
                {s < 3 && <div className={`h-px w-12 transition-colors ${step > s ? 'bg-amber-400' : 'bg-gray-200'}`} />}
              </div>
            ))}
        ...
        {step === 1 && (
          <div className="animate-fade-in">
            <div className="card mb-6">
              <h2 className="font-display text-xl font-semibold text-gray-900 mb-1">Session Configuration</h2>
              <p className="text-gray-500 text-sm mb-6">Set the total number of participants and round duration.</p>

              <div className="mb-6">
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Total participants <span className="text-gray-400 font-normal">(12–50)</span>
                </label>
                <input
                  type="number"
                  min={12}
              ...
              <div className="mb-6">
                <label className="block text-sm font-semibold text-gray-700 mb-2">Round duration</label>
              ...
              <div className="mb-6">
                ...
                >
                  <span className={`w-4 h-4 rounded border flex items-center justify-center text-xs transition-colors ${customizeEvents ? 'bg-amber-500 border-amber-500 text-white' : 'border-gray-300'}`}>
                    {customizeEvents ? '✓' : ''}
                  </span>
                ...
                {customizeEvents && (
                  <div className="mt-3 space-y-2 animate-fade-in">
                    <p className="text-xs text-gray-400">Replace the 3 default events (Press Conference, Product Launch, Internal Conference) with your own.</p>
                    ...
                    {customizeEvents && !customEvents.every(e => e.trim()) && (
                      <p className="text-xs text-amber-600">All 3 event names must be filled to use custom events.</p>
                    )}
                  </div>
                )}
              </div>
              ...
              {config && (
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 mt-4">
                  <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-4">Suggested Configuration</p>
                  <ConfigDiagram config={config} />
                  <div className="mt-4 grid grid-cols-3 gap-4 text-center">
                    <div>
                      <div className="text-2xl font-display font-bold text-amber-500">{config.instances}</div>
                      <div className="text-xs text-gray-500 mt-1">Parallel Instance{config.instances !== 1 ? 's' : ''}</div>
                    </div>
                    <div>
                      <div className="text-2xl font-display font-bold text-gray-900">{config.teamsPerInstance}</div>
                      <div className="text-xs text-gray-500 mt-1">Teams / Instance</div>
                    </div>
                    <div>
                      <div className="text-2xl font-display font-bold text-gray-900">{config.totalTeams}</div>
        ...
        {step === 2 && config && (
          <div className="animate-fade-in">
            <div className="card mb-6">
              <h2 className="font-display text-xl font-semibold text-gray-900 mb-1">Country Selection</h2>
              <p className="text-gray-500 text-sm mb-6">Assign a country to each team slot across all instances.</p>

              {Array.from({ length: config.instances }).map((_, instIdx) => (
                <div key={instIdx} className={`${instIdx > 0 ? 'mt-8 pt-8 border-t border-gray-200' : ''}`}>
                  ...
                  <div className="grid gap-3">
                    {Array.from({ length: config.teamsPerInstance }).map((_, teamIdx) => {
                      ...
                      return (
                        <div key={teamIdx} className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                          <p className="text-xs text-gray-400 font-semibold mb-2">Team {teamIdx + 1}</p>
                          ...
                          {selected && (
                            <p className="text-xs text-gray-500 mt-2">{COUNTRY_STYLES[selected]}</p>
                          )}
                        </div>
                      );
                    })}
        ...
        {step === 3 && sessionData && (
          <div className="animate-fade-in">
            <div className="card mb-6">
              <h2 className="font-display text-xl font-semibold text-gray-900 mb-1">Session Ready</h2>
              <p className="text-gray-500 text-sm mb-6">Share the join codes with participants. Session starts when you click "Start Session" in the dashboard.</p>

              <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-5">
                <p className="text-xs text-amber-700 uppercase tracking-wider font-semibold mb-2">Facilitator Code</p>
                <div className="font-display text-4xl font-bold tracking-widest text-amber-600">
                  {sessionData.facilitatorCode}
                </div>
                <p className="text-xs text-gray-500 mt-1">Keep this private — use to access the facilitator dashboard</p>
              </div>

              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-3">Team Join Codes</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {sessionData.allTeams.map(t => (
                    <div key={t.teamId} className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
                      <div className="text-xs text-gray-500 mb-1">
                        {COUNTRY_FLAGS[t.country]} {t.country} · Inst. {t.instanceNum}
                      </div>
                      <div className="font-display text-2xl font-bold tracking-widest text-gray-900">
                        {t.joinCode}
                      </div>
                    </div>
                  ))}
  ...
  return (
    <div className="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
      <div className="px-4 py-2.5 border-b border-gray-200">
        <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Round timeline</p>
      </div>
        ...
        {events.map((ev, i) => (
          <div key={i} className="flex items-start gap-3 relative">
            ...
            <div className="flex-1 pb-3">
              ...
              <p className="text-xs text-gray-400 mt-0.5 italic">{ev.sublabel}</p>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="flex gap-3 justify-center flex-wrap">
      {Array.from({ length: config.instances }).map((_, i) => (
        <div key={i} className="bg-white rounded-lg p-3 border border-gray-200">
          <p className="text-xs text-gray-400 text-center mb-2">Instance {i + 1}</p>
          <div className="flex gap-1.5 flex-wrap justify-center">
            {Array.from({ length: config.teamsPerInstance }).map((_, t) => (
              <div key={t} className="w-8 h-8 rounded bg-gray-100 border border-gray-200 flex items-center justify-center text-xs text-gray-600 font-medium">
                T{t + 1}
              </div>
            ))}
          </div>
...
function ProjectorView({ sessionData, onClose, onDashboard }) {
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 flex flex-col items-center justify-center p-8 overflow-auto">
      <button onClick={onClose} className="absolute top-5 right-6 text-gray-400 hover:text-gray-700 text-sm transition-colors">
        Exit Fullscreen ✕
      </button>

      <div className="font-display text-5xl font-bold text-gray-900 mb-1 text-center">CrossWire</div>
      <div className="text-gray-500 text-lg mb-10">Scan or type your team's code to join</div>
      ...
      >
        {sessionData.allTeams.map(t => {
          ...
          return (
            <div key={t.teamId} className="bg-gray-50 border border-gray-200 rounded-2xl p-5 flex flex-col items-center gap-3 shadow-sm">
              <div className="flex items-center gap-2">
                <span className="text-2xl">{COUNTRY_FLAGS[t.country]}</span>
                <div className="text-left">
                  <div className="font-display font-bold text-gray-900 text-base">{t.country}</div>
                  {sessionData.allTeams.some(x => x.instanceNum !== t.instanceNum) && (
                    <div className="text-gray-400 text-xs">Instance {t.instanceNum}</div>
                  )}
                </div>
              </div>
      ...
      <div className="text-gray-400 text-sm mb-6">{window.location.origin}/join</div>
